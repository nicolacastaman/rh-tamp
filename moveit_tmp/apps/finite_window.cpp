...
{
  std::string table_name = "table", table_reference_frame = world_frame_;
...
int main(int argc, char** argv)
{
  ...
  // inizializzo collisioni
  /*
  moveit_tmp::CurrentState cs(robot_model_);
  planning_scene::PlanningScenePtr scene, scene_out;
  cs.compute(scene);

  mps.allowCollisions("table", "panda_link0");
  mps.allowCollisions("table", "panda_link1");
  mps.apply(scene, scene_out);
  moveit_msgs::PlanningScene ps;
  scene_out->getPlanningSceneMsg(ps);
  psi_->applyPlanningScene(ps);
  */
  ...
  */
  ...
  while (ros::ok())
  {
    ...
    // TODO: invalidare solo conoscenze non più valide
    if (status_changed)
    {
    ...
    for (int i = j; i < task_solution.size() && i < j + horizon_; i++)
    {
      ...
      else if (action_name == "putdown")
      {
        ...
        if (removed < 8)
          tf = {0.0 + 0.075 * (removed - 1), -0.3, 0.101, 0.0, 0.0, 0.0};
        else
          tf = {0.0 + 0.075 * (removed - 8), 0.3, 0.101, 0.0, 0.0, 0.0};
        ...
        std::vector<std::string> collisions;
        bool res = place.reason(current_scene, world_frame_, poses,
